// script.js (전체 코드 - 수정 및 삭제 기능 포함 최종)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const saveRecordBtn = document.getElementById('saveRecordBtn'); // 저장/수정 버튼
  ...
  // --- 초기화 ---
  populateHourOptions(startHourEl); populateMinuteOptions(startMinuteEl); populateHourOptions(endHourEl); populateMinuteOptions(endMinuteEl);
  ...
  function handleStaffListClick(event) { 
    const clickedStaffName = event.target.dataset.staffName; console.log("[handleStaffListClick] Staff list clicked:", clickedStaffName); 
  ...
  async function fetchStaffNames() { 
    showLoader(); try { console.log("[fetchStaffNames] Fetching staff info..."); const response = await fetch(`${APPS_SCRIPT_WEB_APP_URL}?action=getStaffInfo`); console.log("[fetchStaffNames] Response status:", response.status, response.ok); if (!response.ok) throw new Error(`[${response.status}] ${response.statusText}`); const result = await response.json(); console.log("[fetchStaffNames] Parsed JSON:", result); if (!result.success) throw new Error(result.error || 'Failed to load staff info from API.'); staffListCache = result.data || []; populateStaffList(); if (staffNameEl) { staffNameEl.innerHTML = '<option value="">선택하세요</option>'; staffListCache.forEach(s => { const option = document.createElement('option'); option.value = s.name; option.textContent = s.name; staffNameEl.appendChild(option); }); } } catch (error) { console.error('Error fetching staff names:', error.message, error.stack); showStatusMessage('직원 목록 로딩 실패: ' + error.message, false); } finally { console.log("[fetchStaffNames] Fetch process finished."); hideLoader(); }
  }
  ...
  function displayWorkRecords(records) {
    ...
    records.forEach(record => {
      if (record && typeof record.date === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(record.date.substring(0,10))) {
        ...
        // console.warn("[displayWorkRecords] 유효하지 않은 날짜 형식의 기록 건너뜀:", record);
      }
    });
    
    // console.log("[displayWorkRecords] 달력 표시용으로 그룹화된 기록:", recordsByDate);
    ...
    Object.keys(recordsByDate).forEach(dateStr => {
      ...
      if (!dayCellContentContainer) {
          // console.warn(`[displayWorkRecords] 날짜 ${dateStr}에 대한 달력 셀 컨테이너를 찾을 수 없습니다.`);
          return; 
      }
      ...
      dayRecords.forEach(record => {
        ...
        const recordIdentifier = {
            date: record.date, name: record.name, 
            workType: record.workType, startTime: record.startTime || "" 
        };
        ...
        // entryEl.appendChild(deleteBtn); // 텍스트 설정 후 마지막에 추가

        let currentTrack;
        if (staffToTrackMap.has(record.name)) currentTrack = staffToTrackMap.get(record.name);
        else if (nextAvailableTrack < MAX_STAFF_PER_DAY_DISPLAY) { currentTrack = nextAvailableTrack; staffToTrackMap.set(record.name, currentTrack); nextAvailableTrack++; }
        else return;
        ...
        if (record.workType === "휴가") {
          ...
          textContentForBar = `${record.name}: 휴가`;
        }
        else if (!record.startTime || !record.endTime || TOTAL_TIMELINE_MINUTES <= 0 || 
            !/^\d{2}:\d{2}$/.test(record.startTime) || !/^\d{2}:\d{2}$/.test(record.endTime) ) {
          textContentForBar = `${record.name}: ${record.workType || '(시간없음)'}`;
          ...
          if (durationMinutes < 0 && recordStartMinutes > recordEndMinutes) { /* 자정 넘김 처리 보류 */ }
          if (durationMinutes < 0) durationMinutes = 0;
          let displayStartOffsetMinutes = Math.max(0, startOffsetMinutes);
          let displayEndOffsetMinutes; let displayDurationMinutes;
          ...
          if (displayDurationMinutes <= 0) {
             textContentForBar = `${record.name}: ${record.workType}`; 
             entryEl.style.position = 'relative'; entryEl.style.width = '100%';
          } else {
            ...
            if (barWidthPx < 20) textContentForBar = '&nbsp;'; 
            else if (barWidthPx < 50) textContentForBar = `${record.startTime.substring(0,2)}-${record.endTime.substring(0,2)}`; 
            else textContentForBar = `${record.startTime}-${record.endTime}`;
          }
        }
        
        if (textContentForBar === '&nbsp;') entryEl.innerHTML = textContentForBar; // HTML 허용
        else entryEl.textContent = textContentForBar; // 텍스트로 설정
        
        entryEl.appendChild(deleteBtn); // 모든 내용 설정 후 마지막에 삭제 버튼 추가
  ...
  async function handleDeleteRecord(identifierString) {
    if (!identifierString) {
        showStatusMessage("삭제할 기록 정보가 없습니다.", false);
        return;
    }
    showLoader();
    try {
        const keyObject = JSON.parse(identifierString);
        const payload = { action: 'deleteWorkRecord', key: keyObject };
        console.log("[handleDeleteRecord] 삭제 요청:", payload);
        ...
        console.log("[handleDeleteRecord] 삭제 API 응답 상태:", response.status, response.ok);

        if (!response.ok) {
            const errorText = await response.text();
            console.error("[handleDeleteRecord] 삭제 API 실패. 상태:", response.status, "응답내용:", errorText);
            throw new Error(`[${response.status}] ${response.statusText}. 서버 응답: ${errorText}`);
        }
        const result = await response.json();
        console.log("[handleDeleteRecord] 삭제 API 파싱된 JSON 결과:", result);

        if (result.success) {
            showStatusMessage(result.message || "기록이 삭제되었습니다.", true);
            renderCalendar(currentDisplayedDate.getFullYear(), currentDisplayedDate.getMonth() + 1);
        } else {
            throw new Error(result.error || "알 수 없는 오류로 삭제에 실패했습니다.");
        }
    } catch (error) {
        console.error('Error deleting record:', error.message, error.stack);
        showStatusMessage('삭제 중 오류 발생: ' + error.message, false);
    } finally {
        hideLoader();
        console.log("[handleDeleteRecord] 삭제 과정 종료.");
    }
  }
  ...
  function openModalForDate(dateStr, existingRecordDataWithIdentifier = null) { 
    if (!workRecordForm || !recordDateEl || !modalTitle || !repeatOnWeekdayCheckbox || !saveRecordBtn) return;
    workRecordForm.reset();
    repeatOnWeekdayCheckbox.checked = false;
    notesEl.value = ""; 

    const repeatLabelContainer = repeatOnWeekdayCheckbox.closest('div'); // 체크박스를 감싸는 div 찾기

    if (existingRecordDataWithIdentifier) { 
      const existingRecord = existingRecordDataWithIdentifier;
      console.log("[openModalForDate] 수정 모드로 열기:", existingRecord);
      modalTitle.textContent = `${existingRecord.date} 근무 기록 수정`;
      recordDateEl.value = existingRecord.date;
      staffNameEl.value = existingRecord.name;
      workTypeEl.value = existingRecord.workType;
      
      if (existingRecord.startTime && /^\d{2}:\d{2}$/.test(existingRecord.startTime)) {
        const [startH, startM] = existingRecord.startTime.split(':');
        startHourEl.value = startH; startMinuteEl.value = startM;
      } else { startHourEl.value = "00"; startMinuteEl.value = "00"; }
      if (existingRecord.endTime && /^\d{2}:\d{2}$/.test(existingRecord.endTime)) {
        const [endH, endM] = existingRecord.endTime.split(':');
        endHourEl.value = endH; endMinuteEl.value = endM;
      } else { endHourEl.value = "00"; endMinuteEl.value = "00"; }
      notesEl.value = existingRecord.notes || "";
      
      workRecordForm.dataset.mode = "edit"; 
      currentEditingRecordOriginalKey = JSON.parse(existingRecord.identifier);
      saveRecordBtn.textContent = "수정";
      if (repeatLabelContainer) repeatLabelContainer.style.display = 'none';

    } else { 
      console.log("[openModalForDate] 추가 모드로 열기, 날짜:", dateStr);
      modalTitle.textContent = `${dateStr} 근무 기록 추가`;
      recordDateEl.value = dateStr;
      staffNameEl.value = ""; 
      workTypeEl.value = "주간"; 
      startHourEl.value = "09"; startMinuteEl.value = "00";
      endHourEl.value = "18"; endMinuteEl.value = "00";
      
      workRecordForm.dataset.mode = "add"; 
      currentEditingRecordOriginalKey = null;
      saveRecordBtn.textContent = "저장";
      if (repeatLabelContainer) repeatLabelContainer.style.display = 'block'; // 또는 'flex'
    }
    toggleTimeFields(); 
  ...
  function openModalForEdit(entryElement) { 
      const recordDataString = entryElement.dataset.recordData;
      const recordIdentifierString = entryElement.dataset.recordIdentifier;
      if (recordDataString && recordIdentifierString) {
          const record = JSON.parse(recordDataString);
          record.identifier = recordIdentifierString; 
          openModalForDate(record.date, record); 
      } else {
          console.error("수정할 레코드 정보를 찾을 수 없습니다.", entryElement.dataset);
          showStatusMessage("레코드 정보를 불러오지 못했습니다.", false);
      }
  }
  ...
  function toggleTimeFields() { 
    ...
    const currentMode = workRecordForm.dataset.mode;
    ...
    const repeatContainer = repeatOnWeekdayCheckbox.parentElement; 
    if (repeatContainer) {
        if (isHoliday || currentMode === 'edit') {
            repeatContainer.style.display = 'none';
            repeatOnWeekdayCheckbox.checked = false; 
        } else { // 추가 모드이고 휴가가 아닐 때만 보임
            repeatContainer.style.display = 'block'; // 또는 이전에 설정한 display 값 (예: 'flex')
        }
    }
  ...
  if(workRecordForm) workRecordForm.addEventListener('submit', async (e) => { 
    ...
    const formMode = workRecordForm.dataset.mode || "add";
    const shouldRepeat = (formMode === "add") && repeatOnWeekdayCheckbox.checked;

    const currentRecordData = { 
      date: recordDateEl.value, name: staffNameEl.value, workType: workTypeEl.value, 
    ...
    if (!currentRecordData.name) {
        showStatusMessage('이름을 선택해주세요.', false);
        hideLoader();
        return;
    }

    let recordsToProcess = []; // 서버로 보낼 최종 기록(들)
    let actionType = "";
    let payload = {};
    ...
    if (formMode === "edit") {
        ...
        payload = {
            action: actionType,
            originalKey: currentEditingRecordOriginalKey, 
            newData: currentRecordData 
        };
        ...
        if (shouldRepeat && currentRecordData.workType !== '휴가') {
            ...
            recordsToProcess.push(currentRecordData); // 입력된 날짜의 기록은 항상 포함
            ...
            for (let day = originalDayOfMonth + 1; day <= daysInMonth; day++) { 
                ...
                if (currentDateInLoop.getDay() === targetDayOfWeek) {
                    recordsToProcess.push({ ...currentRecordData, date: `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}` });
                }
            }
        } else {
          recordsToProcess.push(currentRecordData);
        }
        payload = { action: actionType, records: recordsToProcess };
        console.log("[FormSubmit] 저장할 기록:", payload);
    }
    ...
    try {
      ...
      console.log(`[FormSubmit] ${formMode} API 응답 상태:`, response.status, response.ok);
      if (!response.ok) { const errorText = await response.text(); console.error(`[FormSubmit] ${formMode} API 실패. 상태:`, response.status, "응답내용:", errorText); throw new Error(`[${response.status}] ${response.statusText}. 서버 응답: ${errorText}`); }
      const result = await response.json(); console.log(`[FormSubmit] ${formMode} API 파싱된 JSON 결과:`, result);
      if (result.success) { showStatusMessage(result.message || (formMode === "edit" ? "수정되었습니다!" : "저장되었습니다!"), true); if(modal) modal.style.display = 'none'; renderCalendar(currentDisplayedDate.getFullYear(), currentDisplayedDate.getMonth() + 1); }
      else { console.error(`[FormSubmit] ${formMode} API 결과 success:false. 오류:`, result.error, "상세:", result.details); throw new Error(result.error || `알 수 없는 오류로 ${formMode === "edit" ? "수정" : "저장"}에 실패했습니다.`); }
    } catch (error) { console.error(`Error ${formMode === "edit" ? "updating" : "saving"} record(s):`, error.message, error.stack); showStatusMessage(`${formMode === "edit" ? "수정" : "저장"} 중 오류 발생: ` + error.message, false); }
  ...
  async function displayStaffStats() { 
    ...
    if (allRecordsForCurrentMonth.length > 0 && allRecordsForCurrentMonth[0]) {
        if (allRecordsForCurrentMonth[0].date && typeof allRecordsForCurrentMonth[0].date === 'string') {
            try {
                const firstRecordDateParts = allRecordsForCurrentMonth[0].date.substring(0,10).split('-');
                const firstRecordYear = parseInt(firstRecordDateParts[0]);
                const firstRecordMonth = parseInt(firstRecordDateParts[1]);
                if (firstRecordYear === year && firstRecordMonth === month) {
                    needsServerFetchForStats = false;
                }
            } catch (e) { console.warn("통계용 캐시 확인 중 날짜 파싱 오류", e); }
        }
    }

    if (needsServerFetchForStats) {
        console.log("[displayStaffStats] 통계용 데이터 새로 가져오기...");
        showLoader();
        try {
            ...
            console.log("[displayStaffStats] 통계용 데이터 로드 완료:", allRecordsForCurrentMonth.length, "개");
        } catch (error) {
        ...
        console.log("[displayStaffStats] 캐시된 월간 데이터 사용:", allRecordsForCurrentMonth.length, "개");
    }

    let filteredRecords = allRecordsForCurrentMonth.filter(r => r.name === selectedStaff);

    if (filteredRecords.length === 0) {
      statsTableContainer.innerHTML = `<p class="stats-placeholder-message">${selectedStaff} 님의 ${year}년 ${month}월 근무 기록이 없습니다.</p>`;
      statsSummaryContainer.innerHTML = "";
      return;
    }
    ...
    filteredRecords.forEach(r => {
      ...
      if (r.date && typeof r.date === 'string' && /^\d{4}-\d{2}-\d{2}/.test(r.date)) {
          try { const dateObj = new Date(r.date + "T00:00:00"); if(!isNaN(dateObj.getTime())) dayOfWeek = weekdaysKO[dateObj.getDay()]; }
          catch(e) { console.warn("요일 계산 중 날짜 파싱 오류(통계):", r.date, e); }
      }
      const hours = parseFloat(r.totalHours) || 0; 
      tableHTML += `<tr><td>${r.date ? r.date.substring(5) : '?'}</td><td class="${dayOfWeek === '토' ? 'saturday-text' : ''} ${dayOfWeek === '일' ? 'sunday-text' : ''}">${dayOfWeek}</td><td>${r.workType||'-'}</td><td>${r.startTime||'-'}</td><td>${r.endTime||'-'}</td><td>${hours > 0 ? hours.toFixed(1) : (r.workType === '휴가' ? '휴가' : '-')}</td><td title="${r.notes||''}" class="notes-cell">${r.notes||''}</td></tr>`;
      if (workTypeSummary.hasOwnProperty(r.workType)) workTypeSummary[r.workType] += hours;
